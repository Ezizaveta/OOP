...
void ListFreqDictionary::addWord(const std::string & word)
{
	if (!word.empty())
	{
...
bool ListFreqDictionary::ResetEnumerator()
{
	if (!Dictionary.empty())
	{
...
std::pair<std::string, size_t> ListFreqDictionary::getNext()
{
	if (Dictionary.empty())
...
std::list<std::pair<std::string, size_t>>::iterator ListFreqDictionary::findWord(const std::string & word)
{
	if (word.empty())
	{
