...
#include "../TrainingSystemData/DataBase.h"
...
{
	{	//try create a specific instance of the class and upcast
		std::shared_ptr<AbstractCreator> a(new Creator<Teacher>());
		EXPECT_NO_THROW(auto b = a->create("b", 1, 1, vector_ids()));
	}

	{	//try to use in container
...
TEST(FactoryTest, SimpleObjects)
{
	...
	for (size_t i = 0; i < 10; i++)
	{
		...
		{
			//Student* t = dynamic_cast<Student*>(students[i].get());
