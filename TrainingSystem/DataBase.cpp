...
{
	if (!is_acutal) items.insert({ item->getID(), item });
	else { throw  std::out_of_range("Data Base is already updated"); }
}
...
void DataBase::update()
{
	...
	if (is_acutal) { return; }
	//обход графов и прописывание связей всех объектов
	for (auto& item : items)
	{
		auto& current_item = item.second;
		auto indexes_of_item = current_item->getIDs();
		//бежим по всем курсам/студентам найденным для данного объекта
		//и добавляем в них запись о нашем объекте
