...
void Course::addItem(std::weak_ptr<Item> item)
{
	...
	if (std::find_if(getStudents().begin(), getStudents().end(), [&concrete_ptr](std::weak_ptr<Student> student)
		{return student.lock()->getID() == concrete_ptr->getID(); }) == getStudents().end())
	{
