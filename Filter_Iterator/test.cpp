...
#include "../FilterIterator/Range.h"
...
public:
	...
	bool operator==(const IsGreaterTen& pred) const { return ten == pred.ten; }
};
...
public:
	bool operator()(int num) { return ten == num; }
	bool operator==(const IsTen& pred) const { return ten == pred.ten; }
};
...
public:
	...
	bool operator==(const IsMyNum& pred) const { return num == pred.num; }
};
...
/*
TEST(Ñomparison, Test)
{
	
	auto ten = IsTen();
	std::optional<IsTen> opt_ten1(ten);
	std::optional<IsTen> opt_ten2(ten);
	EXPECT_EQ(opt_ten1, opt_ten2);

	
	int ten_ = 10;
	std::optional<int> opt_ten1_(ten_);
	std::optional<int> opt_ten2_(ten_);
	EXPECT_EQ(opt_ten1_, opt_ten2_);
	
	EXPECT_TRUE(std::is_default_constructible_v<IsTen>);
	std::list<int> v1{ 1, 4, 32, 10, 323, 4, 2, 132, 12, 10 };
	IsMyNum num(32);
	auto f1 = make_filter_iterator
		(is_even, v1.begin(), v1.end());
	auto f2 = make_filter_iterator(is_four, v1.begin(), v1.end());
	EXPECT_FALSE(f1 == f2);
}*/
