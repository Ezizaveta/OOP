...
/*
template<typename T>
inline bool operator==(const std::optional<T>& t1, const std::optional<T>& t2)
{
    bool one_is_empty = (!t1.has_value()) && (t2.has_value()) || (t1.has_value()) && (!t2.has_value());
    if (one_is_empty) { return false; }

    bool both_is_empty = (!t1.has_value()) && (!t2.has_value());
    if (both_is_empty) { return true; }
