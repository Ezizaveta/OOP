...
namespace merge
{
	...
	namespace
	{
		...
		void myMerge(Iterator first, Iterator split, Iterator last, Compare comp)
		{
			...
			{
				auto iter = split;
				first = std::find_if(first, split, [&split, &comp](const typename std::iterator_traits<Iterator>::value_type &it) {return comp(*split, it); });// find split < it
				split = std::find_if(split, last, [&first, &comp](const typename std::iterator_traits<Iterator>::value_type &it) {return comp(*first, it); }); // find first < it
		...
		void mergeSort(Iterator first, Iterator last, Compare comp)
		{
			...
			mergeSort(first, split, comp);					//sort left side
			mergeSort(split, last, comp);					//sort right side
			myMerge(first, split, last, comp);				//merge
		}
	}
