...
{
	/* the Krusckal's algorithm sorts edges by weight
	   then checks if the cyrcle is created with current edge.
	   i.e. if 2 vertices of this edge belong to same parent*/
	if (graph.verticesSize() == 0)
	{
		throw std::invalid_argument("Size of the graph is incorrecct!");
	}
	if (graph.edgesSize() == 0)
	{
		throw std::invalid_argument("Number of graph's edges is incorrecct!");
	}
	...
	std::vector<std::pair<double, edge>>::iterator it = unique(Edges.begin(), Edges.end());
