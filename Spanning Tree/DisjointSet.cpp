...
{
	if (n == 0) throw std::invalid_argument("Number of Vertices is incorrect! ");
	size = n;
...
size_t DisjointSet::find(size_t u)
{
	...
	if (u > size) throw std::out_of_range("incorret id in function find!");
	if (u != parent[u])
...
void DisjointSet::setUnion(size_t v1, size_t v2)
{
	...
	if (( v1 > size) || (v2 > size))
		throw std::out_of_range("incorret id to set union!");
