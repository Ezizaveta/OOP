...
#include "../RoadMarking/RoadMarker.h"
#include "../RoadMarking/Writer.h"
...
TEST(Parser, GetLineInvalid1)
{
	std::istringstream ss{
			...
			"10 changing\n" };
	Parser parser(ss);
	Line expect1{ 10, changing };
	std::pair<std::vector<Line>, bool> lines;
	EXPECT_ANY_THROW(lines = parser.getLines());
}
...
TEST(RoadMarker, Complex1)
{
	std::istringstream ss{
			...
			"4 changing\n"
			"10 continuous\n" };
	std::vector<Command> expected = { Command{0.0, down},
		Command{ 10.0, up }, 
		Command{ 13.0, down},
		Command{ 14.0, up },
		Command{ 15.0, down },
...
TEST(RoadMarker, Complex2)
{
	std::istringstream ss{
			...
			"9 changing\n"
			"10 empty\n" };
	std::vector<Command> expected = { Command{1.0, down},
		Command{ 2.0, up },
		Command{ 5.0, down},
		Command{ 6.0, up },
		Command{ 8.0, down },
...
{
	Command c1{ 1.0, up };
	Command c2{ 20.0, down };
	Command c3{ 24.0, up };
	Command c4{ 27, down };
	std::vector<Command> result = {c1, c2, c3, c4};
