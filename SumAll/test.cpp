...
TEST(Vector, String)
{
	...
	ASSERT_EQ(sumAll(v1), "abcdef");  //ASSERT_STREQ почему-то использовать не хочет
...
{
	std::map<int, char> v1 = { {1, 'a'}, {2, 'b'}, {3, 'c'} }; //почему это работает? 
	ASSERT_EQ(sumAll(v1), "abc");
}
...
TEST(Map, CharToInt)
{
	std::map<int, unsigned char> v1 = { {1, 'a'}, {2, 'b'}, {3, 'c'} }; //почему работает? 
	ASSERT_EQ(sumAll(v1), 294);
...
{
	std::map<int, std::string> v1 = { {1, "ab"}, {2, "cd"}, {3, "ef"} }; //”ра, не работает! 
...
{
	std::unordered_map<int, std::string> v1 = { {1, "ab"}, {2, "cd"}, {3, "ef"} }; //”ра, не работает! 
	ASSERT_EQ(sumAll(v1), "abcdef");
