...
TEST(Erase, Key)
{
	...
	EXPECT_EQ((int)l1.size(), const_size);
	l1.erase(8);
	EXPECT_EQ((int)l1.size(), const_size - 1);
	std::cout << "2 " << l1;
...
TEST(Erase, ManyIterator)
{
	...
	EXPECT_EQ((int)l1.size(), const_size/2);
...
TEST(Erase, Range)
{
	...
	EXPECT_EQ((int)l1.size(), const_size - 6);
